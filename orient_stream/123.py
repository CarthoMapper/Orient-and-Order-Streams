...
GG = nx.read_shp(r'C:\Users\vital\Desktop\Diplom\vectors\flip_hokkaido.shp')
...
nx.write_shp(G, r'C:\Users\vital\Desktop\Diplom\vectors\Ordering')
...
nx.set_node_attributes(GG, 0,'USSR')
nx.set_node_attributes(GG, 0,'Horton')
...
GU = GG.to_undirected(as_view=True)
A = (GU.subgraph(c) for c in nx.connected_components(GU))
A = list(A)
...
weighted_G = nx.Graph()
for data in GG.edges(data=True):
    weighted_G.add_edge(data[0], data[1], weight=data[2]['length'])

nx.diameter(weighted_G)
nx.dag_longest_path(weighted_G, , weight = 'length')

sub_G = nx.DiGraph()
create_subgraph(G, sub_G, node)

for i in range(1, len(sp_e)):
            G.nodes[sp_e[i]]['Horton'] = G.nodes[sp_e[i-1]]['Horton']
            if G.degree[sp_e[i]] > 2:
                nbrs = [n for n in G.neighbors(sp_e[i])]
                inflow = []
                inflow.extend(nbrs)
                ...
                if len(nbrs) > 2:
                    inflow.remove(sp_e[i+1])
                    nbrs.remove(sp_e[i+1])
                G.nodes[sp_e[i]]['Horton'] = max(G.nodes[sp_e[i-1]]['Horton'], G.nodes[inflow[0]]['Horton'])
                if G.nodes[inflow[0]]['Horton'] == 0:
                    continue
                if G.nodes[inflow[0]]['Horton'] == G.nodes[sp_e[i-1]]['Horton']:
                    G.nodes[sp_e[i]]['Horton'] = (G.nodes[sp_e[i-1]]['Horton'] + 1)

per = nx.periphery(weighted_G)
attrs = []
for i in GG.edges():
    temp = [('USSR', GU.nodes()[i[0]]['USSR']),('Horton', GU.nodes()[i[0]]['Horton']),('Shrive', GU.nodes()[i[0]]['Shrive']),('Stahler', GU.nodes()[i[0]]['Stahler']),('Rzhanitsyn', GU.nodes()[i[0]]['Rzhanitsyn']),('Scheideg', GU.nodes()[i[0]]['Scheideg'])]
    d = dict(temp)
    attrs.append((i, d))
attrs = dict(attrs)
nx.set_edge_attributes(GG, attrs)
nx.write_shp(GG, r'C:\Users\vital\Desktop\Diplom\vectors\Ordering')